...
class Runner(object):
    ...
    def evaluate(self, split, epoch):
        ...
        results:			The evaluation results containing the following:
            ...
            results['hits@k']:      Probability of getting the correct prediction
            in top-k ranks based on predicted score
        ...
        self.logger.info(
            '[Epoch {} {}]: MRR: Tail : {:.5}, Head : {:.5}, Avg : {:.5}'.format(
                epoch, split, results['left_mrr'], results['right_mrr'], results['mrr']
            ))
        if split == 'test':
            self.logger.info('hits@1: {:.3}, hits@3: {:.3}, hits@10: {:.3}'.format(results['hits@1'], results['hits@3'],
                                                                                   results['hits@10']))
        return results
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(train_iter):
                ...
                ranks = 1 + torch.argsort(
                    torch.argsort(pred, dim=1, descending=True), dim=1, descending=False)[b_range, obj]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-savedir', dest='save_dir', default='./checkpoints', help='Checkpoint directory')
    parser.add_argument('-savefinal', dest='savefinal', action='store_true', help='save final model to pickle file')
    parser.add_argument('-using_gat', dest='using_gat', action='store_true', help='Use graph attention')
